defmodule OllamaService do
  ...
  def pull_model_if_not_available(model_name) do
    ...
    # Check if auto model pulling is allowed
    auto_pull_models = GHF.get_auto_pull_ollama_models_setting()

    if not auto_pull_models do
      IO.puts("Model pulling disabled by configuration. Skipping pull for '#{model_name}'.")
          ...
          if auto_quantized do
            case select_optimal_model_for_vram(model_name, available_models) do
              {:ok, optimal_model} when optimal_model != case_insensitive_match ->
                IO.puts(
                  "VRAM optimization: using '#{optimal_model}' instead of '#{case_insensitive_match}'"
                )
  ...
  defp get_nvidia_vram do
    case System.cmd("nvidia-smi", ["--query-gpu=memory.free", "--format=csv,noheader,nounits"],
           stderr_to_stdout: true
         ) do
      {output, 0} ->
  ...
  defp estimate_model_vram_mb(model_name) do
    ...
    cond do
      ...
      String.contains?(model_lower, "70b") or String.contains?(model_lower, "72b") ->
        # ~140GB for fp16
        estimate_by_quantization(model_lower, 140_000)
      ...
      String.contains?(model_lower, "30b") or String.contains?(model_lower, "33b") or
        ...
        # ~70GB for fp16
        estimate_by_quantization(model_lower, 70_000)
  ...
  defp select_optimal_model_for_vram(base_model_name, available_models) do
    ...
    suitable_models =
      ...
      # Descending - prefer higher quality
      |> Enum.sort_by(fn {_model, vram_req} -> -vram_req end)
  ...
  defp extract_base_model_name(model_name) do
    model_name
    # Remove _q8_0, _q4_k_m, etc.
    |> String.replace(~r/_q\d+.*$/i, "")
    # Remove :latest tag
    |> String.replace(~r/:latest$/i, "")
  end
